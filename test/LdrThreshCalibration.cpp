...
namespace {
    ...
    constexpr gpio_num_t LDR_PIN = GPIO_NUM_34; // -> ADC_UNIT_1 / ADC_CHANNEL_6, see AdcGpioMapping.cpp
...
TEST_CASE("LDR threshold calibration: converges on a physically plausible ambient/lit split", "[LdrThreshCalibration]") {
    ...
    // ambient ~800, laser hit ~9000, 150ms rise/fall - well inside the 500ms pulse period so
    // readings are always settled by the time we verify
    randomStub.test_setSeed(1234);
    LdrPhysicsSim ldrSim(800, 9000, 150);
    ...
    const int calibratedThreshold = module.getLdrThreshold();
    REQUIRE(calibratedThreshold > 800);
    REQUIRE(calibratedThreshold < 9000);
    REQUIRE(calibratedThreshold != 3000); // CALIB_LDR_THRESH_INITIAL_THRESH
}
...
TEST_CASE("LDR threshold calibration: faults immediately when ambient light already swamps the initial threshold", "[LdrThreshCalibration]") {
    ...
    // ambient (4000) is already above the initial threshold (3000), so it can't tell on from
    // off even on the first batch - no known-good value to fall back to, real failure
    randomStub.test_setSeed(1234);
    LdrPhysicsSim ldrSim(4000, 9000, 150);
...
TEST_CASE("LDR threshold calibration: a rise time close to the pulse period causes misreads", "[LdrThreshCalibration]") {
    ...
    // threshold yet, so it reads as a false positive. this is exactly what
    // CALIB_LDR_TRESH_PULSE_FREQ is supposed to give headroom against
    randomStub.test_setSeed(1234);
    LdrPhysicsSim ldrSim(800, 9000, 900);
